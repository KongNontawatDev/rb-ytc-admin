  ...
  return new Promise((resolve, reject) => {
    try {
      loader.file.then(async (file: File) => {
        ...
        if (response) {
          const imageUrl = axios.getUri() + "static" + path + "/" + response.data.results.filename
          resolve({default: imageUrl, filename: response.data.results.filename})
        }
      })
    } catch (error) {
      reject(error)
...
export function MyUploadAdapter(loader: { file: Promise<File> }, setLoading: (loading: boolean) => void, path: string, field: string) {
  // Define the upload function
  const upload = async () => {
    setLoading(true);
    return ckEditorUploadImage(loader, field, path)
      .then((response) => {
        setLoading(false);
        return response;
      })
