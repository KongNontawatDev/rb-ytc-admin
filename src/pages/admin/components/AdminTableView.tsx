...
import { useUpdateSelectedStatusAdmin } from "../hooks/useAdminMutate";
...
export const AdminTableView: React.FC<{
  data: Admin[];
  ...
  const updateStatusMutation = useUpdateSelectedStatusAdmin(()=>{});
  ...
  const handleUpdateStatus = async() => {
    if (newStatus !== undefined) {
      ...
      setErrorSubmit({message:t("validation:placeholderSelect",{field:t("admin:schema.status.label")})} as any)
    }
  };
      ...
      >
        ...
        <Spin spinning={updateStatusMutation.isPending}>
          <ActiveStatusDropdown
            ...
            placeholder={t("common:select",{data:t("admin:schema.status.label")})}
          />
        </Spin>
      </Modal>
    </>
