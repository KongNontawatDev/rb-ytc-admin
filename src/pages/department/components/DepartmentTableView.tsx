...
import { useUpdateSelectedStatusDepartment } from "../hooks/useDepartmentMutate";
...
export const DepartmentTableView: React.FC<{
  data: Department[];
  ...
  onChange: (
    ...
    extra: TableCurrentDataSource<Department>
  ) => void;
  ...
  const { t } = useTranslation(["common","department","validation"]);
  const updateStatusMutation = useUpdateSelectedStatusDepartment(()=>{});
  ...
  const handleUpdateStatus = async() => {
    if (newStatus !== undefined) {
      ...
      setErrorSubmit({message:t("validation:placeholderSelect",{field:t("department:schema.status.label")})} as any)
    }
  };
      ...
      >
        ...
        <Spin spinning={updateStatusMutation.isPending}>
          <ActiveStatusDropdown
            ...
            placeholder={t("common:select",{data:t("department:schema.status.label")})}
          />
        </Spin>
      </Modal>
    </>
