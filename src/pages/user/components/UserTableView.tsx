...
import { useUpdateSelectedStatusUser } from "../hooks/useUserMutate";
...
export const UserTableView: React.FC<{
  ...
  const updateStatusMutation = useUpdateSelectedStatusUser(()=>{});
  ...
  const handleUpdateStatus = async() => {
    if (newStatus !== undefined) {
      ...
      setErrorSubmit({message:t("validation:placeholderSelect",{field:t("user:schema.status.label")})} as any)
    }
  };
      ...
      >
        ...
        <Spin spinning={updateStatusMutation.isPending}>
          <ActiveStatusDropdown
            ...
            placeholder={t("common:select",{data:t("user:schema.status.label")})}
          />
        </Spin>
      </Modal>
    </>
